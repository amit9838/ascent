import {
  ...
  solvedThisWeek,
  weekRangeLabel,
} from "../../lib/gamification/activity.ts";
import { useWeeklyTarget } from "../../lib/entities/settings.ts";
import { dailyTarget } from "../../lib/gamification/rewards.ts";
import { pointsOf } from "../../lib/gamification/points.ts";
import {
  CalendarIcon,
  CheckIcon,
  FlameIcon,
...
export function fmtDay(key: string /* YYYY-MM-DD, parsed without timezone shift */): string {
...
export function OverviewRow({ solves, points, total, solved }: DashboardProps) {
  const streak = currentStreak(solves);
  const week = solvedThisWeek(solves, points);
  const best = bestDay(solves, points);
  const [target] = useWeeklyTarget();
  const totalPts = Object.entries(solves).reduce(
...
export function WeeklyGoalCard({ solves, points, total, solved }: DashboardProps) {
  const [target] = useWeeklyTarget();
  const week = solvedThisWeek(solves, points);
  const pct = target > 0 ? Math.min(100, Math.round((week / target) * 100)) : 0;
  const dt = dailyTarget(target);
  const todayCount = dayPoints(solves, points)[dayKey()] ?? 0;
...
function cellColor(points: number, future: boolean): string {
...
export function ActivityHeatmap({
  ...
  const cols = heatmapWeeks(solves, points, 15);
  return (
    <section className={`${card} h-full`}>
