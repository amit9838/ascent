...
// Local note shape as used by the notes page (superset-friendly: extra
// fields like isDeleted pass through untouched).
export interface LocalSharedNote {
  id: string;
  sharedId?: string;
  shared?: boolean;
  ownerUid?: string;
...
function selfInfo(user: User): SharedMemberInfo {
  return {
...
async function lookupUid(email: string): Promise<{ uid: string | undefined; email: string }> {
  ...
  return { uid: snap.data()?.uid as string | undefined, email: addr };
}
...
export async function shareNoteWith(
  ...
  let members: string[];
  let memberInfo: Record<string, SharedMemberInfo>;
  if (snap?.exists()) {
    const d = snap.data() ?? {};
...
export async function unshareMember(
  ownerUser: User | null,
  note: LocalSharedNote,
  targetUid: string
): Promise<string[]> {
  ...
  const members = (Array.isArray(d.members) ? d.members : []).filter(
    (u: unknown) => u !== targetUid && u !== ownerUser.uid
  );
  await step(
...
export async function stopSharing(ownerUser: User | null, note: LocalSharedNote): Promise<void> {
  if (!ownerUser) throw new Error("Sign in to manage sharing.");
  const { db, m } = await fs();
  ...
  const members: unknown[] = snap?.exists()
  ...
  await Promise.all(
    members
      .filter((u): u is string => typeof u === "string" && u !== ownerUser.uid)
      .map((u) =>
...
export async function leaveSharedNote(user: User | null, note: LocalSharedNote): Promise<void> {
  if (!user) return;
  const { db, m } = await fs();
  const shareId = note.sharedId || note.id;
...
export async function listSharedInbox(uid: string): Promise<SharedInboxEntry[]> {
  if (!uid) return [];
  const { db, m } = await fs();
  const snap = await m
...
export async function fetchSharedNote(
  shareId: string
): Promise<(SharedNoteDoc & { shareId: string }) | null> {
  if (!shareId) return null;
  const { db, m } = await fs();
  const snap = await m.getDoc(m.doc(db, "sharedNotes", shareId)).catch(() => null);
  if (!snap?.exists()) return null;
  return { shareId, ...(snap.data() ?? {}) } as SharedNoteDoc & { shareId: string };
}
...
export async function pushSharedNote(note: LocalSharedNote, user: User | null): Promise<void> {
  ...
  if (note.ownerUid === user.uid || (!note.ownerUid && !note.shared)) {
    const snap = await m.getDoc(m.doc(db, "sharedNotes", shareId)).catch(() => null);
    const members: unknown[] = snap?.exists() && Array.isArray(snap.data()?.members)
      ? snap.data().members
      : [];
    await Promise.all(
      members
        .filter((u): u is string => typeof u === "string" && u !== user.uid)
        .map((u) =>
          m
            .setDoc(
...
export function subscribeSharedInbox(
  uid: string,
  onChange: (items: SharedInboxEntry[]) => void,
